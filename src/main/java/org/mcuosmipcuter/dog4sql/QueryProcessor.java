...
public class QueryProcessor extends Worker
{
	...
	public QueryProcessor(QueryListener queryListener, String strSQL, Statement stmt)
	throws Exception
	{
	...
	public void runImpl()
	{
		ResultWrapper rw = new ResultWrapper(strSQL, stmt);
		...
		try
		{
			...
			if(rw.getResultSet() != null)
			{
				if(process){
					...
					Controller.trace(3, "counting rows ...");
					rw.countRows();
		...
		finally
		{
			...
			rw = null;
			process = false;
			Controller.trace(4, this + " resources released.");
		}
